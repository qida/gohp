...
// https://github.com/holdno/snowFlakeByGo/blob/master/snowFlake.go
// 因为snowFlake目的是解决分布式下生成唯一id 所以ID中是包含集群和节点编号在内的
const (
	// workerBits uint8 = 10 // 每台机器(节点)的ID位数 10位最大可以有2^10=1024个节点
	workerBits uint8 = 7 // 每台机器(节点)的ID位数 6位最大可以有2^6=128个节点
	...
	epoch int64 = 1658678400000 // 2022-07-25 00:00:00 起始时间戳(毫秒)
)
...
type SnowFlake struct {
	...
	number    int64      // 当前毫秒已经生成的id序列号(从0开始累加) 1毫秒内最多生成32768个ID
}
